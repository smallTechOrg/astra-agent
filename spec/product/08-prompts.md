...
Prompts are first-class spec artifacts, not source code constants. They live in text files and are loaded at runtime. This lets the operator tune tone without code deploys, and lets each tenant override for brand voice.

## Resolution order

For prompt `{name}` when running tenant `<id>`:

1. `config/tenants/<id>/prompts/{name}.txt` — tenant override
2. `prompts/{name}.txt` — operator default
...
Prompts are plain text with Python-style `{placeholder}` substitution. The first line of the file *may* be a machine-readable header comment declaring variables:
...
Rest of the file is the prompt, possibly including a `---` separator between system and user parts:
...
When the `---` separator is present, content above becomes the LLM `system_prompt` and content below becomes the `user_prompt`. When absent, the whole file is the user prompt and system is null.
...
## Operator-default prompt files

These **must** exist in `prompts/` for Astra to start at all, even if no tenant uses them:

- `prompts/linkedin_announcement.txt`
- `prompts/twitter_announcement.txt`

Cadence prompts are named by cadence — no universal default required, but every configured cadence must have either an operator default or a per-tenant override. Absence at startup is a fatal error for that tenant.

## Versioning prompts

Prompts are tracked in git like any other source file. Git history is the prompt changelog. No in-file version strings.

Changing a prompt's **variables** is a breaking change — the spec section above must be updated in the same commit. Changing a prompt's **wording** is a routine edit, no spec change needed.
